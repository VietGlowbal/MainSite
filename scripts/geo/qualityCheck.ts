...
function hasGenericAiPhrases(body: string) {
  return /(in today's competitive world|navigate the complex landscape|journey of studying abroad|unlock your potential)/i.test(body);
}
...
function evaluate(slug: string, markdown: string): GeoQualityCheck {
  ...
  const duplicateScore = Math.max(
    0,
    ...[...listExistingPageSlugs()].filter((item) => item !== slug).map((item) => cosineLikeSimilarity(item, slug)),
  );
  ...
  const hasNoOfficialSources = officialSourceCount === 0;
  const tooManyTodos = todoSourceCount > 3;
  ...
  if (hasTodoSources) {
    score -= Math.min(15, todoSourceCount * 4);
  ...
  if (hasTuitionNumbersWithoutSource) blockerReasons.push('Contains specific tuition or cost numbers without a linked source.');
  if (hasEntryNumbersWithoutSource) blockerReasons.push('Contains specific IELTS or entry requirement numbers without a linked source.');
  if (rankingLanguageWithoutMethodology) blockerReasons.push('Uses ranking language without a methodology section.');
  if (hasNoOfficialSources) blockerReasons.push('No official sources linked for this page.');
  if (tooManyTodos) blockerReasons.push('More than 3 TODO_SOURCE_REQUIRED markers remain.');
  if (hasTodoSources) blockerReasons.push('Unresolved TODO_SOURCE_REQUIRED markers remain.');
  ...
  const passed = score >= 80 && duplicateRisk !== 'high' && hasShortAnswer && hasMethodology && hasFaqs && hasGlowbalCTA;
  const publishable = passed && blockerReasons.length === 0 && officialSourceCount > 0;
  const reviewRequired = !publishable;
