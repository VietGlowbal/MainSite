...
import type { DraftManifest, GeoPageMetadata, GeoQualityCheck, StudentQuestion, TopicCluster } from './types';
...
export function listExistingPageSlugs() { const slugs = new Set<string>(); for (const base of [path.join(repoRoot,'src/app'), path.join(repoRoot,'content/geo/drafts')]) { if (!fs.existsSync(base)) continue; walk(base, (filePath) => { const relative = path.relative(base, filePath).replace(/\\/g, '/'); if (relative.endsWith('/page.tsx')) { const slug = relative.replace(/\/page\.tsx$/, ''); if (slug) slugs.add(slug); } if (relative.endsWith('.md')) slugs.add(path.basename(relative, '.md')); }); } return slugs; }
export function cosineLikeSimilarity(a: string, b: string) { const tokens = (value: string) => new Set(value.toLowerCase().split(/[^a-z0-9]+/).filter((token) => token.length > 2)); const aTokens = tokens(a); const bTokens = tokens(b); const intersection = [...aTokens].filter((token) => bTokens.has(token)).length; return intersection / Math.max(aTokens.size + bTokens.size - intersection, 1); }
