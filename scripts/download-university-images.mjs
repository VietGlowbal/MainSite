...
async function existingFileMetadata(filePath, outputPath, publicBaseUrl) {
  ...
  return {
    ...
    optimized: extension === 'webp' ? 'yes' : 'unknown',
    sourceBytes: '',
    sourceWidth: '',
    sourceHeight: '',
...
async function normalizeImage(image, assetKey, options) {
  ...
  // Rasterize SVG logos as part of the normal pipeline. Publishing an SVG URL
  // would require Next's dangerouslyAllowSVG option wherever logo_url is
  // rendered through next/image. A high input density keeps crests and small
  // lettering crisp before they are fitted into the 512px output canvas.
  const pipeline = sharp(image.buffer, {
    animated: true,
    ...(image.extension === 'svg' ? { density: 300 } : {}),
  }).rotate();
  const resized =
    assetKey === 'logo'
...
async function downloadAsset(task, options) {
  ...
  if (!options.force) {
    ...
    if (existing) {
      const metadata = await existingFileMetadata(
        existing,
        options.outputPath,
        options.publicBaseUrl,
      );
      const expectedWidth = assetKey === 'logo' ? options.logoSize : options.campusWidth;
      const expectedHeight = assetKey === 'logo' ? options.logoSize : options.campusHeight;
      const alreadyNormalized =
        metadata.contentType === 'image/webp' &&
        metadata.outputWidth === expectedWidth &&
        metadata.outputHeight === expectedHeight;

      if (!options.optimize || alreadyNormalized) {
  ...
  try {
    ...
    if (options.force || options.optimize) {
...
function createUpdateSql(rows, bucket) {
  ...
  const values = usableRows
    ...
    .join(',\n');
  const imageDataSource = usableRows.length
    ? `values\n${values}`
    : `select null::text as id, null::text as image_url, null::text as logo_url\n  where false`;
