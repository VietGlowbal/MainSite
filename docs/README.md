...
| [known-issues.md](known-issues.md) | Before touching `/universities`, `/my-universities`, `/mentors`, saving, auth, analytics — **or any `sql/supabase-*.sql` file**. §0 is the migration trap; §1b is the mentorship RLS gap; §7 is why `.env.example` documents nothing; §8 is what GA4 does and does not measure. |
