...
Working tree 2026-09-26 (reference-report style grounding): reviewed all four supplied VINUNI DOCX personal-report samples through parallel report analyses and separated their editorial guidance from executable facts. The shared report flow now instructs the GPT-6 Luna synthesis prompt to extract only supplied facts, group them by framework section, pair evidence with interpretation, preserve supported/emerging/hypothesis status, use a causal arrow chain when supported, keep one canonical anchor experience plus corroborating titles, and state growth validation only when the gap is evidenced. Prompt version is `report-synthesis-v18-reference-style-anchor-grounding`; narrative materialization canonicalizes and persists the anchor/supporting experience titles, and the positioning UI renders them. The narrative parser now routes omitted available framework sections through targeted repair instead of silently accepting partial model JSON, while deterministic fallback keeps every visible framework part present; narrative telemetry reports `complete` only when all seven structured narrative keys are present. Verified focused personal-report tests (3 files, 79 passed), narrative/domain regression tests after the completeness guard (47 + 22 passed), `npm.cmd run typecheck:strict`, touched-file ESLint, `node scripts/check-i18n.mjs --all`, `git diff --check`, and `npm.cmd run build` (successful; build emitted environmental Supabase quota errors during static data access and three `src/lib/geo-content.ts` tracing warnings). No production data or deployment was changed; pre-existing unrelated working-tree changes remain preserved.
