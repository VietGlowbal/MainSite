...
- Course-parser async reliability & watchdog architecture:
  - Additive watchdog reaper (`reapStaleParseJobs` in `src/lib/course-parser/job-queue.ts`) integrated into `/api/cron/process-parse-jobs`: safely identifies processing leases inactive for >5 minutes, reclaims recoverable jobs (`attempts < max_attempts`) to `pending` with exponential backoff and sets application `parse_status: 'pending'`, and marks exhausted or deadlocked jobs as `failed` with actionable user-facing messages. Reconciles stranded application records.
  - Granular parse phases (`src/lib/course-parser/parse-phases.ts`): provides coherent execution phases `queued -> fetching -> extracting -> validating -> ready` (and `timeout` / `failed`), while safely mapping to/from legacy `pending / processing / complete / timeout / failed` strings.
  ...
  - Idempotent retry API (`/api/applications/[id]/retry-parse`): preserves completed output (idempotent 200 without overwriting data), prevents duplicate active jobs (idempotent 200 on pending and non-stale processing), allows retries on failed/timeout/stale (>5 min) work, uses the correct queued `pending` state when re-enqueued, and enforces a rate limit window (max 3 retries/hour).
  - UI & polling responsiveness (`use-parse-refresh.ts` and `my-application-section.tsx`): detects stale or timeout phases and triggers refresh immediately; UI replaces indefinite "AI is reading..." with actionable "Taking longer than usual... Try again" and error messaging with retry actions.
  - SQL safety: created additive migration `sql/supabase-course-parse-reliability.sql` adding nullable `phase` column and watchdog index on `course_parse_jobs (status, updated_at)` without destructive operations or assumptions of live deployment.
- Measured:
  - 8 test files / 60 tests passed across course-parser and apply domains (`parse-phases.test.ts`, `job-queue.test.ts`, `retry-parse/route.test.ts`, `parse-status/route.test.ts`, `process-parse-jobs/route.test.ts`, `use-parse-refresh.test.tsx`, `extract-course.test.ts`, `baseline-checklist.test.ts`).
