...
- Measured:
  - 8 test files / 62 tests passed across course-parser and apply domains (`parse-phases.test.ts`, `job-queue.test.ts`, `retry-parse/route.test.ts`, `parse-status/route.test.ts`, `process-parse-jobs/route.test.ts`, `use-parse-refresh.test.tsx`, `extract-course.test.ts`, `baseline-checklist.test.ts`).
