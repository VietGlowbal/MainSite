...
Last reconciled: **2026-08-14 (Asia/Ho_Chi_Minh)**
...
|---|---|---|
| *(uncommitted)* | **Added deterministic Data-Adaptive University-Programme Matching v1.** The authenticated `/universities/matches` route ranks the existing programme catalogue using an explainable, versioned scoring model: admission evidence (GPA, tests, prerequisites, selectivity and degree level) remains separate from destination/field/budget/city preferences. It links `catalog_programmes` to `courses` only through `source_programme_id`, consumes normalized catalogue v2 programme facts with scope/audience/cycle/verification/null-reason safeguards, uses university-level requirements/selectivity only as explicitly labelled fallbacks/proxies, and never lets crawler-extracted or source-excerpt evidence create a hard eligibility failure. Sparse or incompatible data is shown as unknown/partial rather than invented. The public university directory gains a compact link into this personalised surface. Added 14 focused deterministic-engine tests, 8 adapter tests (including normalized prerequisite and fallback precedence cases), and normalization coverage for the onboarding study-level labels used by admission scoring. | Students can compare programmes through their own profile with ranked, explainable results, evidence limitations, coverage, and Strong Chance/Target/Reach tiers where evidence coverage is sufficient. The matching result is not presented as an admission probability, and no fabricated admission data was introduced. |
...
## Matching v1 verification (2026-08-14)

| Gate | Result |
|---|---|
| `npm test -- src/features/universities/domain/matching/matching.test.ts src/features/universities/api/__tests__/matching-candidate-loader.test.ts` | **Pass:** 14 deterministic matching tests and 8 adapter tests. |
| `npm test -- src/features/universities` | **Pass:** 211 tests, 2 todo across the university feature cluster. |
| Feature-scoped ESLint | **Pass:** the route, matching API/domain/UI, and directory link have 0 errors. |
| `npm run typecheck:strict` | Blocked by an existing missing `mammoth` module in `src/lib/ai/document-text.ts`; the matching code has no strict-type errors. |
| `npm run typecheck` | Blocked by a locked `tsconfig.tsbuildinfo`, stale generated `.next` route validators, and existing missing `mammoth`/`@react-pdf/renderer` dependencies. |
| `npm test` | **Fail:** 1,703 passed, 20 failed, 2 todo across 166 passing and 5 failing files. The failures are pre-existing dependency/import failures for `mammoth` and `@react-pdf/renderer`, plus the existing i18n checker integration failure; the matching suite passed as part of the run. |
| `npm run build` | Blocked before compilation by `EPERM` opening `.next/trace-build` in this workspace. |
...
| `npx vitest run` | **Pass: 1718 passed, 2 todo, 169 files passed, 0 failed.** New `application-nav-background.test.tsx` (renders a hidden canvas, unmounts cleanly, renders nothing under `prefers-reduced-motion`). |
