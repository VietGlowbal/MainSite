...
Code snapshot: branch `claude/branch-comparison-review-l6e2tu` at `ac1117d`,
pushed and open as PR #172 (not yet merged) against `main` at `e61545e` (PR
#169, "Size the header animation to fill its height…", merged). See "Last
...
- All twelve Candidate Information questions are now rebuilt (specs 1–3, all
  pushed to PR #172): 1–4 (education/nationality/scores), 5–8 (subjects,
  countries, study level, intake), and 9–12 (aspirations, per-subject
  motivation, funding, tuition budget). **Two migrations are still outstanding**
  and both PATCH/read paths degrade gracefully without them:
  `supabase-reflection-questions.sql` (from #171 — `study_motivation`,
  `target_intake`) and `supabase-reflection-subject-motivations.sql` (from
  this work — `subject_motivations`, a JSONB map keyed by subject id).
