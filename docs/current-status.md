...
Code snapshot: branch `claude/branch-comparison-review-l6e2tu` at `a57f0f1`,
built on `main` at `6568818` (PR #172, "Redesigned the Candidate Information
questionnaire," merged — the branch was restarted from `main` after the merge
rather than stacking on the now-squashed pre-merge history). Not yet pushed as
a PR. See "Last completed work" below.
...
- All twelve Candidate Information questions are rebuilt and merged (PR #172,
  ...
  rebuilt as an upload-first card grid — see the latest row below. **Three
  migrations are still outstanding** and every PATCH/read path degrades
  gracefully without them: `supabase-reflection-questions.sql` (from #171 —
  `study_motivation`, `target_intake`), `supabase-reflection-subject-
  motivations.sql` (from spec 3 — `subject_motivations`, a JSONB map keyed by
  subject id), and `supabase-reflection-review-status.sql` (from this work —
