...
- The working tree has real uncommitted work: the Candidate Information
  questionnaire redesigned to the owner's spec — two display modes, option
  cards, a searchable flag picker over all 197 nationalities, real score
  scales with inline validation, and AI grade/English conversion. No migration
  needed for it (provenance rides in `grades_summary`), but
