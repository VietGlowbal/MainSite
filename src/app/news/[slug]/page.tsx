        ...
        <div className="rounded-[2rem] border border-white/80 bg-white/95 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] md:p-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500">
              <span className="rounded-full bg-pink-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-pink-600"><T k={guide.topic} /></span>
              <span>{formatDate(guide.publishedAt)}</span>
              <span>•</span>
              <span><T k="{minutes} min read" vars={{ minutes: guide.readingTimeMinutes }} /></span>
              <span>•</span>
              <span className="inline-flex items-center gap-1.5">
                <T k="By Glowbal Editorial Team" />
          ...
          <AutoTranslate as="h1" className="mt-6 max-w-4xl text-4xl font-semibold tracking-tight text-slate-900 md:text-[3.35rem] md:leading-[1.1]" text={guide.title} />
          {guide.description ? <AutoTranslate as="p" className="mt-4 max-w-3xl text-xl leading-9 text-slate-600" text={guide.description} /> : null}
          ...
          {guide.keyTakeaway ? (
            <section className="mt-6 flex gap-4 rounded-[1.7rem] bg-[linear-gradient(135deg,rgba(125,95,255,0.08),rgba(255,77,140,0.07))] p-5">
              ...
              <div>
                <h2 className="text-lg font-semibold text-slate-900"><T k="Key takeaway" /></h2>
                <AutoTranslate as="p" className="mt-1 text-lg leading-8 text-slate-600" text={guide.keyTakeaway} />
              </div>
            </section>
          ) : null}
          ...
          {guide.supportCards.length > 0 ? (
            <section className="mt-10">
              <h2 className="text-3xl font-semibold tracking-tight text-slate-900"><T k="Why this matters" /></h2>
                ...
                {guide.supportCards.map((card, index) => {
                  ...
                  return (
                    <article key={card.title} className="rounded-[1.4rem] border border-slate-200 bg-white p-5 shadow-sm">
                      ...
                      <AutoTranslate as="h3" className="mt-4 text-xl font-semibold text-slate-900" text={card.title} />
                      <AutoTranslate as="p" className="mt-2 text-slate-600 leading-7" text={card.description} />
                    </article>
                  );
                })}
              </div>
        ...
        <aside className="space-y-5">
          {guide.toc.length > 0 ? (
            <section className="rounded-[1.8rem] border border-white/80 bg-white/95 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
              <h2 className="text-lg font-semibold text-slate-900"><T k="On this page" /></h2>
              <ol className="mt-4 space-y-3 text-sm text-slate-600">
                {guide.toc.map((item, index) => (
                  <li key={item.id} className="flex gap-3">
                    <span className={index === 0 ? 'text-pink-500 font-semibold' : 'text-slate-400'}>{index + 1}.</span>
                    <a href={`#${item.id}`} className={index === 0 ? 'text-pink-500' : 'hover:text-slate-900'}><AutoTranslate text={item.title} /></a>
                  </li>
                ))}
              </ol>
            </section>
              ...
              {related.map((item) => (
                <Link key={item.slug} href={`/news/${item.slug}`} className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <AutoTranslate as="div" className="text-sm font-semibold leading-6 text-slate-900" text={item.title} />
                    <div className="mt-1 text-xs text-slate-500"><T k="{minutes} min read" vars={{ minutes: item.readingTimeMinutes }} /></div>
                  </div>
                </Link>
              ))}
            </div>
            <Link href="/news" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-pink-600"><T k="View all articles" /> <span aria-hidden>→</span></Link>
          </section>
