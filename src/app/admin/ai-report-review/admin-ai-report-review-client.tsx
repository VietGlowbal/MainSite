...
function StructuredDataView({ value, omitIdentity = false, depth = 0 }: { value: unknown; omitIdentity?: boolean; depth?: number }) {
  ...
  return <dl className="grid gap-gb-sm">{entries.map(([key, child]) => <div key={key} className={depth > 1 ? 'border-l border-line pl-gb-lg' : ''}><dt className="text-gb-xs font-semibold text-fg-muted">{humanize(key)}</dt><dd className="mt-gb-xxs text-gb-sm text-fg-secondary">{Array.isArray(child) && child.length > 4 ? <details><summary className="cursor-pointer text-fg">{child.length} items</summary><div className="mt-gb-sm"><ValueList values={child} omitIdentity={omitIdentity} /></div></details> : scalar(child) !== null && /status|alignment|classification|kind/i.test(key) ? <span>{humanize(String(scalar(child)))}</span> : <StructuredDataView value={child} omitIdentity={omitIdentity} depth={depth + 1} />}</dd></div>)}</dl>;
}
...
function StrategyRenderer({ output }: { output: RecordValue }) {
  ...
  const priorities = Array.isArray(overview?.topPriorities) ? overview.topPriorities.map((priority) => { const item = priority as RecordValue; const factors = item.factors && typeof item.factors === 'object' ? { ...(item.factors as RecordValue) } : null; if (factors) delete factors.rawPriority; return { ...item, ...(factors ? { factors } : {}) }; }) : null;
  const activities = Array.isArray(profile?.activityAnalyses) ? profile.activityAnalyses : [];
  return <div className="flex flex-col gap-gb-2xl"><Section title="Strategic overview"><StructuredDataView value={overview} /></Section><Section title="Priorities"><StructuredDataView value={priorities} /></Section><Section title="Profile development"><StructuredDataView value={profile?.areas} /></Section><Section title="Activities"><div className="flex flex-col gap-gb-sm">{activities.length ? activities.map((activity, index) => <details key={index} className="rounded-gb-xl border border-line p-gb-lg"><summary className="cursor-pointer font-semibold text-fg">{String((activity as RecordValue).title ?? `Activity ${index + 1}`)}</summary><div className="mt-gb-sm"><StructuredDataView value={activity} /></div></details>) : <span className="text-fg-muted">Not available</span>}</div></Section><Section title="Narrative strategy"><StructuredDataView value={output.narrativeStrategy} /></Section><Section title="Four-phase roadmap"><div className="grid gap-gb-md md:grid-cols-2">{roadmap.map((phase, index) => <details key={index} className="rounded-gb-xl border border-line p-gb-lg"><summary className="cursor-pointer font-semibold text-fg">{humanize(String((phase as RecordValue).name ?? (phase as RecordValue).phaseKey ?? `Phase ${index + 1}`))}</summary><div className="mt-gb-sm"><StructuredDataView value={phase} /></div></details>)}</div></Section><Section title="Evidence & sources"><details><summary className="cursor-pointer text-gb-sm text-fg">Show evidence index</summary><div className="mt-gb-md"><StructuredDataView value={{ evidence: output.evidenceIndex, sources: output.targetSourceIndex }} /></div></details></Section></div>;
}
function LegacyRenderer({ output }: { output: unknown }) {
  return <div className="flex flex-col gap-gb-xl"><div className="rounded-gb-xl border border-line bg-surface-subtle p-gb-lg text-gb-sm text-fg-secondary"><p className="font-semibold text-fg">Legacy or partially validated output</p><p className="mt-gb-xs">The report contract is older or contains historical references that no longer validate. The stored content is shown below in a readable form; raw JSON remains available in Technical.</p></div><StructuredDataView value={output} /></div>;
}
