...
function getScholarshipValueScore(opt: ScholarshipOption): number {
  if (opt.amountMax && opt.amountMax > 0) return opt.amountMax;
  if (opt.amountMin && opt.amountMin > 0) return opt.amountMin;
  if (opt.coverage) {
    const match = opt.coverage.match(/(\d+)%/);
...
function ScholarshipPicker({
  ...
  // For non-Plus accounts with > 3 candidates, pick 1 highest + 2 lowest
  let visibleCandidates = candidates;
  let blurredCandidates: Candidate[] = [];

  if (!isPlus && candidates.length > 3) {
