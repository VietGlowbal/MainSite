...
export async function GET() {
  ...
  const { data: profiles } = await admin
    ...
    .select('user_id, is_admin, onboarding_completed, study_level');
...
export async function PATCH(request: NextRequest) {
  ...
  if (parsed.data.user_id === guard.user!.id && !parsed.data.is_admin) {
  ...
  // Upsert is_admin onto student_profiles. If the user has never completed
  // onboarding there might not be a row yet, so we upsert.
  const { error } = await admin
    .from('student_profiles')
    .upsert(
      { user_id: parsed.data.user_id, is_admin: parsed.data.is_admin },
      { onConflict: 'user_id' },
    );
