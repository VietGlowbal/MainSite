...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase.from('payment_transactions').select('id, reference, provider, product_type, status, amount_vnd, expires_at, paid_at, fulfilled_at, booking_id, plus_plan').eq('reference', reference).eq('user_id', user.id).maybeSingle();
  ...
  return NextResponse.json({ ...data, status, status_label: manualStatusLabel(status), can_claim: canClaim, bank_label: config.bankLabel, account_holder: config.accountHolder, account_number: config.accountNumber, account_number_masked: config.accountNumberMasked, bank_qr_url: config.bankQrUrl });
}
