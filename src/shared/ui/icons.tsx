 ...
 */
/**
 * The show/hide-password toggle glyph.
 *
 * Lifted here on the third call site (sign-in, reset, change password). It is
 * always paired with a control that has its own `aria-label`, so the svg stays
 * hidden from assistive tech — the state is announced by the button, not drawn.
 */
export function EyeMark({ off = false, frame = 18 }: { off?: boolean; frame?: number }) {
  return (
    <svg
      width={frame}
      height={frame}
