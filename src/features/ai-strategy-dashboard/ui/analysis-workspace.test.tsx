...
const PERSONAL_GET = '/api/applications/app-1/strategy/applicant-analysis';
...
describe('AnalysisWorkspace', () => {
  it('generates both reports and transitions to "reports are ready" once both complete', async () => {
    const fetchMock = vi.fn((url: string, init?: RequestInit) => {
      if (url === PERSONAL_GET && !init) return jsonResponse({ analysis: null });
      if (url === PERSONAL_GET && init?.method === 'POST') return jsonResponse({ analysis: { id: 'p1' } });
    ...
    expect(screen.getByRole('link', { name: 'View my reports' })).toHaveAttribute(
      'href',
      '/ai-strategy/app-1/strategy/analysis/portrait',
    );
    expect(screen.queryByText('Generating…')).not.toBeInTheDocument();
  });

  it('lets a finished report be opened while the other is still generating', async () => {
    let resolveMatching: (() => void) | null = null;
    const fetchMock = vi.fn((url: string, init?: RequestInit) => {
      if (url === PERSONAL_GET && !init) return jsonResponse({ analysis: { id: 'p1' } });
      if (url === MATCHING_GET && !init) {
        return new Promise<Response>((resolve) => {
          resolveMatching = () => resolve({ json: () => Promise.resolve({ analysis: null }) } as Response);
        });
      }
    ...
    // Matching is still mid-flight — only one row has finished.
    expect(screen.getAllByText('Generating…')).toHaveLength(1);
    expect(resolveMatching).not.toBeNull();
  });

  it('shows an inline error with its own retry for a report that fails, independent of the other', async () => {
    ...
    // Personal succeeded independently of Matching's failure.
