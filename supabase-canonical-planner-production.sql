...
-- Apply AFTER supabase-core3-plan-hierarchy.sql. This is additive: it never
-- deletes legacy recommendations or canonical execution history.

-- Clients keep read-only access through the existing ownership SELECT policies.
-- All planning and execution writes go through authenticated server routes,
-- which use the service-role client after server-side entitlement/ownership
-- checks. This prevents direct PostgREST mutation of planning-owned fields.
REVOKE INSERT, UPDATE, DELETE ON public.application_plans FROM anon, authenticated;
REVOKE INSERT, UPDATE, DELETE ON public.application_plan_phases FROM anon, authenticated;
REVOKE INSERT, UPDATE, DELETE ON public.application_plan_steps FROM anon, authenticated;
REVOKE INSERT, UPDATE, DELETE ON public.application_plan_micro_steps FROM anon, authenticated;
      ...
      FOR v_micro IN SELECT value FROM jsonb_array_elements(COALESCE(v_step->'microSteps', '[]'::jsonb)) LOOP
        INSERT INTO application_plan_micro_steps(step_id, domain_node_id, title, sort_order, readiness, content_schema, source_decision_ids, source_provenances, archived_at)
          ...
          ON CONFLICT (step_id, domain_node_id) DO UPDATE SET title = EXCLUDED.title, sort_order = EXCLUDED.sort_order,
            ...
            status = CASE WHEN application_plan_micro_steps.content_schema IS DISTINCT FROM EXCLUDED.content_schema
              AND application_plan_micro_steps.content_value IS NULL THEN 'not_started' ELSE application_plan_micro_steps.status END,
