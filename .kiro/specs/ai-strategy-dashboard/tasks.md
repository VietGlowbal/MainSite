...
**Recommendation-engine correctness pass** (`fix/recommendation-engine-correctness`, after a review of the merged Phase 4-6 work): the first version of the recommendation generator computed `estimatedImpact`/`pillar` but never persisted them (the columns didn't exist), and deduplicated purely by title, which meant a regenerate could only skip-if-exists — never update a changed recommendation, retire one the AI no longer produces, or tell a real generation failure apart from "genuinely nothing to recommend." `supabase-strategy-recommendation-fields.sql` adds `estimated_impact`, `pillar`, `source_analysis_id`, `archived_at`. `domain/recommendation.ts#reconcileRecommendations` (with unit tests) replaces the title-only dedup: matches by (pillar, title), updates a still-relevant non-completed row in place, leaves a completed one completely untouched, archives (never deletes) one no longer represented. `generateRecommendations` now returns a typed result the Dashboard surfaces as a real error banner instead of an unexplained empty table.

**Onboarding-completion-state correctness pass** (`fix/onboarding-completion-state`, second subset of the same audit): the original "has at least one achievement or activity" proxy for `reflectionComplete` could never be satisfied by a student with genuinely zero achievements (Requirement 4.3 explicitly allows finishing empty) and was satisfied by any row regardless of which flow wrote it. `supabase-strategy-onboarding-state.sql` adds `student_profiles.personal_summary_completed_at` / `achievements_completed_at` (shared, stamped on that step's explicit submit — not inferred from row counts) and `course_applications.strategy_intro_seen_at` (per-Strategy, stamped when the Intro page is opened). `domain/onboarding.ts` (with unit tests) is the new single source of truth: `nextOnboardingStep` decides both "what's first for a new student" and "where does a partially-onboarded student resume" with one function, and `isOnboardingComplete` is what Requirement 1.3 checks before skipping Strategy Home entirely and going straight to the Dashboard. Strategy Home, the analysis page, and the Dashboard page all now redirect a student who lands on the wrong stage to the correct one instead of rendering broken or empty state — closing the "prevent users from accidentally entering later stages without required data" gap. Two inaccurate copy claims caught in the same pass ("your strategy updates automatically") were corrected to describe the real, manual "Re-analyse now" action instead.

Both passes are a genuine, reviewed subset of a much larger audit the project owner provided (21 sections covering live database/AI verification, continuous optimisation, CV-branch integration, Figma/accessibility, and production readiness) — the rest is tracked outside this file since most of it needs the owner, credentials, or tooling this session doesn't have, not more code from here.

**Owner-supplied loading video + English-only copy** (direct owner instruction, not from the written audit): the owner supplied a real loading animation clip, transcoded from the original 31MB/2026x1410/60fps `.mov` to `public/ai-strategy-loading.mp4` (858KB, 960x668, 30fps, no audio) plus a poster frame, using the same muted/loop/`playsInline`/reduced-motion-fallback treatment `GlobeLoader`'s `Globe` already established for `/loading-globe.mp4` (`usePrefersReducedMotion` exported from `globe-loader.tsx` for reuse rather than duplicated). It now plays on loop during `AnalysisWorkspace`'s "generating" state — the same 30-60s wait Requirement 5.1's cycling text messages already covered, now with real motion behind them. Separately, the owner asked that Personal Summary and Achievements be English-only: both forms (and the shared `ReflectionShell` header they sit in) had inherited Vietnamese section titles and button labels from the older `/ai-strategy/reflection` flow this spec extended rather than replaced (CLAUDE.md's general EN-VI-bilingual default is superseded here by this specific, direct instruction) — every section title, field label, placeholder and button on both screens is English now.

**Live bug found and fixed 2026-07-31: `application_recommendations` had no INSERT RLS policy.** A user-reported screenshot showed the Dashboard completely empty ("We couldn't refresh your recommendations just now", no rows) for a student with a completed Course Match Analysis. `supabase-apply-v2.sql` enabled RLS on `application_recommendations` with a SELECT and an UPDATE policy but never an INSERT one, so `generateRecommendations`'s `.insert(...)` — run on the request-scoped, RLS-respecting client — has been rejected since the Dashboard shipped in #112; nothing else in the codebase writes to this table, so no student could ever get a first recommendation row created, and it read as "AI matching completely broken." Fixed with an additive `supabase-strategy-recommendations-insert-policy.sql` (owner must run it — see `docs/known-issues.md §0b`) plus `generateRecommendations`'s `logMigrationHint`, which now distinguishes this from a missing-column error (`PGRST204`) in server logs instead of a bare "insert failed."
