-- Forward-only canonical Planner task-guidance migration. Apply after:
--   1) supabase-core3-plan-hierarchy.sql
--   2) supabase-canonical-planner-production.sql
--   3) supabase-planner-ops.sql
--   4) supabase-planner-production-hardening.sql
--   5) supabase-planner-production-hardening-multi-microstep-fix.sql
-- It adds planning-owned student-facing guidance without altering execution
-- fields, and reinstalls the terminal hardened reconciler as the final writer.

ALTER TABLE public.application_plan_micro_steps
...
BEGIN
  ...
  END IF;
  -- The application row is the cross-instance mutex for canonical reconciliation.
  ...
  ELSE
    UPDATE application_plans
      SET domain_plan_id = p_plan->>'domainPlanId', readiness = p_plan->>'readiness', updated_at = v_now
      WHERE id = v_plan_id;
  ...
  FOR v_phase IN SELECT value FROM jsonb_array_elements(COALESCE(p_plan->'phases', '[]'::jsonb)) LOOP
    ...
    FOR v_step IN SELECT value FROM jsonb_array_elements(COALESCE(v_phase->'steps', '[]'::jsonb)) LOOP
      ...
      FOR v_micro IN SELECT value FROM jsonb_array_elements(COALESCE(v_step->'microSteps', '[]'::jsonb)) LOOP
        INSERT INTO application_plan_micro_steps(step_id, domain_node_id, title, guidance, sort_order, readiness, content_schema, source_decision_ids, source_provenances, archived_at)
          VALUES (v_step_id, v_micro->>'domainNodeId', v_micro->>'title', NULLIF(v_micro->>'guidance', ''), (v_micro->>'order')::integer,
          ...
          ON CONFLICT (step_id, domain_node_id) DO UPDATE SET title = EXCLUDED.title, guidance = EXCLUDED.guidance,
            sort_order = EXCLUDED.sort_order, readiness = EXCLUDED.readiness,
            status = CASE
