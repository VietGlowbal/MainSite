-- Repair the canonical Planner reconciler installed by
-- supabase-canonical-planner-production.sql.
--
-- The original function reused v_step_id for the id returned by each inserted
-- micro-step. A second micro-step then received the first micro-step's id as
-- its parent step_id and failed the application_plan_micro_steps_step_id_fkey.
